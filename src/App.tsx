...
import { formatDuration, formatDateDMY } from "./utils";
...
export default function App() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (res && res.user) {
          setAccessLevel(res.user.role || "Technician");
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      fetchNodes();
      fetchNotifications();
    }, 30000); // Refresh every 30s to avoid rate limits
  ...
  useEffect(() => {
    if (
      showDetailedReport &&
      selectedNodeId &&
      detailedReportStartDate &&
      detailedReportEndDate
    ) {
  ...
  const fetchConfig = async () => {
    ...
    try {
      ...
      const emailData = await fetchJson("/api/email-settings", {
        headers: { "x-access-level": accessLevel },
      });
      if (emailData) {
  ...
  const confirmResetLifetime = async () => {
  ...
  const sinceDate = formatDateDMY(selectedNode.lifetime_anchor_date);
                          ...
                          >
                            ...
                            <div className="space-y-1">
                              ...
                              <span className="text-[10px] text-slate-400">
                                {`${formatDateDMY(n.created_at)} ${new Date(
                                  n.created_at
          ...
          <div className="flex gap-6">
            ...
            <div className="flex items-center pl-4 border-l border-slate-200">
              <button
                ...
                className="text-xs font-bold text-slate-500 hover:text-red-600 uppercase tracking-widest transition-colors flex items-center gap-1"
              >
                Logout
        ...
        <div className="flex flex-1 min-h-[550px]">
          ...
          <div className="flex-1 p-8 flex flex-col bg-white">
            ...
            <div className="grid grid-cols-3 gap-6 mb-8 max-w-3xl">
              ...
              <div
                className="bg-[#f1f5f9] rounded-2xl p-6 aspect-square max-h-48 flex flex-col justify-center items-center shadow-sm border border-slate-200 relative overflow-hidden cursor-pointer hover:bg-slate-200 transition-colors"
                onClick={() => {
                  setShowDetailedReport(true);
                  // We'll add a way to open filament tab directly by using a state or similar if needed.
                  // For now it opens the modal.
                  setTimeout(() => {
                    const btn = document.getElementById("tab-filament");
                    if (btn) btn.click();
                  }, 100);
                }}
              >
                ...
                <History className="w-10 h-10 text-purple-400 mb-2" />
                <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase">
                  LOGS
                </h3>
                <span className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">
                      ...
                      logs.map((log) => (
                        <tr key={log.id} className="text-slate-700">
                          <td className="px-4 py-3 whitespace-nowrap">
                            {`${formatDateDMY(log.created_at)} ${new Date(
                              log.created_at
                            ).toLocaleTimeString("en-US", {
                              hour: "2-digit",
                              minute: "2-digit",
              ...
              >
                <div className="p-6">
                  ...
                  <p className="text-sm text-slate-500">
                    Are you sure you want to reset the filament lifetime
                    counter? This will log the previous duration in the history
                    and restart the count from 0.
                  </p>
                </div>
          ...
          {showSettingsModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0.9, opacity: 0 }}
                className="relative w-full max-w-4xl max-h-[90vh] flex flex-col bg-white rounded-[32px] shadow-2xl overflow-hidden"
              >
                    ...
                    <div className="space-y-6">
                      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            className={`w-full px-4 py-3 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 ${!isEditingTelegram ? "bg-slate-50 border-transparent text-slate-500 cursor-not-allowed" : "bg-white border-slate-200 shadow-sm"}`}
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                        ...
                        <button
                          ...
                          className="w-full py-2.5 bg-[#e0e7ff] text-blue-700 font-bold text-xs rounded-xl uppercase tracking-wider hover:bg-blue-100 transition mt-2"
                        >
                          Send Test Alert
                        </button>
                      </div>
                      ...
                      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                        ...
                        <p className="text-xs text-slate-500 leading-relaxed">
                          Clicking below clears current telemetry and fills the
                          database with 45 days of realistic mock telemetry
                          (including guaranteed data for today and yesterday).
                          This lets you test the Excel daily reports with full
                          attachments.
                        </p>
                        <button
                          onClick={async () => {
                            if (
                              !window.confirm(
                                "This will overwrite current telemetry logs with realistic simulated data (including records for today). Proceed?"
                              )
                            )
                              return;
                            try {
                              ...
                              if (res?.success) {
                                alert(
                                  "Database seeded successfully with 45 days of realistic data! Refreshing..."
                                );
                          ...
                          className="w-full py-2.5 bg-blue-50 text-blue-700 hover:bg-blue-100 font-bold text-xs rounded-xl uppercase tracking-wider transition"
                        >
                          Generate 45 Days of Mock Data
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="space-y-6">
                      <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                        <div className="border-b border-slate-50 pb-3 flex justify-between items-center">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="text-[10px] font-bold bg-slate-100 hover:bg-slate-200 text-slate-600 px-2 py-1 rounded"
                            >
                              Set Gmail
                            </button>
                            <button
                            ...
                            <button
                              onClick={async () => {
                                try {
                                  await fetchJson("/api/generate-ethereal", {
                                    method: "POST",
                                    headers: { "x-access-level": accessLevel },
                                    body: JSON.stringify({
                                      emails:
                                  ...
                                  fetchConfig();
                                  alert(
                                    "Generated free Ethereal test account! Try sending a test email now."
                                  );
                                } catch (err) {
                                  alert("Failed to generate test account.");
                                }
                              }}
                              className="text-[10px] font-bold bg-purple-100 hover:bg-purple-200 text-purple-700 px-2 py-1 rounded"
                            >
                              Set Ethereal (Free Test)
                            </button>
                          </div>
                        </div>
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                          ...
                          <input
                            type="time"
                            value={emailSettings.schedule_time || "20:00"}
                            onChange={(e) => {
                              const newSettings = {
                                ...emailSettings,
                          ...
                          <div className="space-y-1">
                            ...
                            <p className="text-[10px] text-slate-500 mt-1 flex flex-wrap gap-1 items-center">
                              ...
                              <button
                                onClick={(e) => {
                                  e.preventDefault();
                                  setShowGmailGuide(!showGmailGuide);
                                }}
                                className="text-purple-600 hover:underline font-bold text-[10px] focus:outline-none"
                              >
                                {showGmailGuide
                                  ? "Hide Setup Steps"
                                  : "Show Setup Steps"}
                            ...
                            {showGmailGuide && (
                              <motion.div
                                ...
                                className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2 text-xs text-slate-600 mt-2 overflow-hidden"
                              >
                                ...
                                <ol className="list-decimal pl-4 space-y-1.5 text-[11px] leading-relaxed">
                                  ...
                                  <li>
                                    Click on <strong>Security</strong> in the
                                    left sidebar.
                                  </li>
                                  <li>
                                    Under "How you sign in to Google", verify
                                    that <strong>2-Step Verification</strong> is
                                    enabled (if not, turn it on first).
                                  </li>
                                  <li>
                                    Search for <strong>"App passwords"</strong>{" "}
                                    in the top search bar (or go directly to{" "}
                                    <a
                                      href="https://myaccount.google.com/apppasswords"
                                      target="_blank"
                                      rel="noreferrer"
                                  ...
                                  <li>
                                    ...
                                    <code>Factory Portal</code>) and click{" "}
                                  ...
                                  <li>
                                    Paste it into the{" "}
                                    <strong>SMTP Password</strong> field here,
                                    and ensure your <strong>SMTP User</strong>{" "}
                                    is your full Gmail address (e.g.,{" "}
                        ...
                        <button
                          onClick={async () => {
                            try {
                              const res = await fetchJson("/api/test-email", {
                                method: "POST",
                                headers: { "x-access-level": accessLevel },
                              });
                              if (res?.success) {
                                if (res.previewUrl) {
                                  alert(
                                    `Test email sent! View preview: ${res.previewUrl}`
                                  );
                              ...
                              alert(
                                `Failed to send test email: ${err.message || "Check console and SMTP settings."}\n\nNote: If you are using Gmail, you may need to use an 'App Password' instead of your regular password.`
                              );
                            }
                          }}
                          className="w-full py-2.5 bg-purple-50 hover:bg-purple-100 text-purple-600 font-bold rounded-xl transition-colors text-xs uppercase tracking-wider mt-2"
                        >
                          Send Test Email
                        </button>
                      </div>
                      ...
                      <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                        <p className="text-xs text-slate-500 leading-relaxed">
                          ...
                          If your bot doesn't send notifications organically,
                          ensure the node has a successful heartbeat first.
                          Background watchdog checks run every minute and
                          respect the configured shift hours.
                        </p>
                      </div>
                    </div>
                  </div>
