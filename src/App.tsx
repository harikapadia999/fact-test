...
export default function App() {
  ...
  const [accessLevel, setAccessLevel] = useState<'Admin' | 'Technician'>('Technician');
  ...
  const [newLog, setNewLog] = useState({ technician_name: '', event_type: 'Maintenance', notes: '' });
  const [config, setConfig] = useState<Record<string, string>>({});

  const [lifetimeHours, setLifetimeHours] = useState<number>(0);
  const [showDetailedReport, setShowDetailedReport] = useState(false);
  const [detailedReportDate, setDetailedReportDate] = useState<string>(new Date().toISOString().split('T')[0]);
  const [detailedReportData, setDetailedReportData] = useState<TelemetryDetails[]>([]);
  ...
  const fetchTelemetry = async (id: string) => {
    try {
      ...
      const processedData = telemetryData.map((d: any) => ({
        ...d,
        hours: d.hours || 0,
        rawDate: d.rawDate || new Date(Date.now() - (telemetryData.length - 1 - telemetryData.indexOf(d)) * 24 * 60 * 60 * 1000).toISOString().split('T')[0] // Approximation for click handler
      }));
  ...
  useEffect(() => {
    if (showDetailedReport && selectedNodeId && detailedReportDate) {
      const fetchDetails = async () => {
        ...
        try {
          const details = await fetchJson(`/api/nodes/${selectedNodeId}/telemetry/details?date=${detailedReportDate}`);
  ...
  const sinceDate = new Date(selectedNode.lifetime_anchor_date).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' });
      ...
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col border border-slate-200">
        ...
        <div className="px-8 py-6 flex items-center justify-between border-b border-slate-100">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-medium text-slate-800 tracking-tight">Factory Data Board</h1>
                        ...
                        notifications.map(n => (
                          ...
                          >
                            <div className={cn(
                              ...
                              n.type === 'safety' ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"
                            )}>
                              {n.type === 'safety' ? <AlertTriangle className="w-4 h-4" /> : <Bell className="w-4 h-4" />}
                            </div>
                            <div className="space-y-1">
                              <p className="text-xs text-slate-700 leading-tight">{n.message}</p>
          ...
          <div className="flex gap-6">
            <div className="flex flex-col">
              ...
              <span className="text-sm font-bold text-slate-900 font-mono tracking-tight">{selectedNode.status === 'online' ? 'ACTIVE' : 'OFFLINE'}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Bulb Usage</span>
              <span className="text-sm font-bold text-slate-900 font-mono tracking-tight">{Math.round(lifetimeHours)} / 1000 hrs</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Since</span>
              <span className="text-sm font-bold text-slate-900 font-mono tracking-tight">{sinceDate}</span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Role</span>
              <span className="text-sm font-bold text-slate-900 font-mono tracking-tight">{accessLevel}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-1 min-h-[550px]">
          ...
          >
            <div className={cn("px-6 py-4 flex items-center justify-between transition-opacity duration-200", isSidebarCollapsed ? "opacity-0" : "opacity-100")}>
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Node List</span>
              {accessLevel === 'Admin' && (
                <button 
                ...
                <div key={node.id} className="group relative">
                  <button
                    ...
                    className={cn(
                      ...
                      isSidebarCollapsed ? "justify-center px-0 border-l-0" : "gap-3",
                      selectedNodeId === node.id 
                        ? "bg-[#e0e7ff] border-blue-600 text-blue-900" 
                        : "border-transparent hover:bg-slate-50 text-slate-700"
                    )}
                    title={isSidebarCollapsed ? (node.alias || node.id) : undefined}
                  >
                    <div className={cn(
                      "w-2.5 h-2.5 rounded-full shrink-0 shadow-sm",
                      node.status === 'online' ? "bg-green-500" : "bg-slate-300"
                    )} />
                    {!isSidebarCollapsed && (
                      <span className="text-sm font-medium truncate flex-1">{node.alias || node.id}</span>
                    )}
                  </button>
          ...
          <div className="flex-1 p-8 flex flex-col bg-white">
                  ...
                  <div className="flex items-center gap-3">
                    {isEditingSelectedAlias && accessLevel === 'Admin' ? (
                      ...
                      <div 
                        ...
                        onClick={() => accessLevel === 'Admin' && setIsEditingSelectedAlias(true)}
                      >
                    ...
                    {selectedNode.xray_active_since && (
                      ...
                      >
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider">Safety Warning</span>
                      </motion.div>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-2 gap-6 mb-8 max-w-2xl">
              <div className="bg-[#f1f5f9] rounded-2xl p-6 aspect-square max-h-48 flex flex-col justify-center items-center shadow-sm border border-slate-200 relative overflow-hidden">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest absolute top-4 left-4">X-Ray Status</span>
                {selectedNode.xray_active_since ? (
                  <>
                    <ShieldAlert className="w-10 h-10 text-red-500 mb-2" />
                    <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase">ACTIVE</h3>
                    <span className="text-[10px] font-medium text-red-500 uppercase tracking-widest">EMITTING RADIATION</span>
                  </>
                ) : (
                  <>
                    <ShieldCheck className="w-10 h-10 text-green-500 mb-2" />
                    <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase">INACTIVE</h3>
                    <span className="text-[10px] font-medium text-green-500 uppercase tracking-widest">SAFE</span>
                  </>
                )}
              </div>
              <div className="bg-[#f1f5f9] rounded-2xl p-6 aspect-square max-h-48 flex flex-col justify-center items-center shadow-sm border border-slate-200 relative overflow-hidden">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest absolute top-4 left-4">Access Control</span>
                <Lock className="w-10 h-10 text-blue-300 mb-2" />
                <h3 className="text-xl font-black text-slate-800 tracking-tight uppercase">{accessLevel}</h3>
                <span className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">{accessLevel === 'Admin' ? 'FULL ACCESS' : 'READ ONLY'}</span>
              </div>
            </div>
            ...
            <div className="flex-1 min-h-[200px] mt-auto flex flex-col">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-medium text-slate-500 block">↑ Usage (Hours)</span>
                <button 
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={telemetry} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="date" axisLine={true} tickLine={false} tick={{ fontSize: 11, fill: '#64748b' }} minTickGap={15} />
                  <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#64748b' }} />
                  <Tooltip cursor={{ fill: '#f8fafc' }} />
                  <Bar dataKey="hours" fill="#1d4ed8" radius={[4, 4, 0, 0]} barSize={analyticsView === 'Weekly' ? 40 : 12}>
                    {analyticsView === 'Weekly' && <LabelList dataKey="hours" position="top" fill="#64748b" fontSize={11} formatter={(val: number) => val.toFixed(1)} />}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              <div className="text-center mt-2 text-[11px] text-slate-500">{analyticsView} Period</div>
            </div>

            {/* Maintenance Ledger */}
            <div className="mt-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-widest">Maintenance Ledger</h3>
                <button 
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {logs.length > 0 ? logs.map(log => (
                      <tr key={log.id} className="text-slate-700">
                        ...
                        <td className="px-4 py-3 font-medium">{log.technician_name}</td>
                        <td className="px-4 py-3">
                          <span className="px-2 py-1 bg-slate-200 text-slate-700 rounded-md text-[10px] font-bold uppercase tracking-wider">
                            {log.event_type}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-slate-500 truncate max-w-[200px]">{log.notes}</td>
                      </tr>
                    )) : (
                      <tr>
                        <td colSpan={4} className="px-4 py-8 text-center text-slate-400 text-xs italic">No maintenance logs found.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        <div className="bg-[#f8fafc] border-t border-slate-200 p-6 flex flex-col gap-6">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className={cn("text-sm font-medium transition-colors", accessLevel === 'Admin' ? "text-slate-800" : "text-slate-400 hover:text-slate-600")}
                >
                  Admin
                </button>
                <button 
                  onClick={() => setAccessLevel('Technician')}
                  className={cn("px-6 py-1.5 rounded-full text-sm font-medium transition-colors border", accessLevel === 'Technician' ? "bg-[#e0e7ff] text-blue-700 border-blue-200" : "bg-transparent text-slate-400 border-transparent hover:text-slate-600")}
                >
                  Technician
                </button>
              </div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className={cn("px-6 py-1.5 rounded-full text-sm font-medium transition-colors border", analyticsView === 'Weekly' ? "bg-[#e0e7ff] text-blue-700 border-blue-200" : "bg-transparent text-slate-400 border-transparent hover:text-slate-600")}
                >
                  Weekly
                </button>
                <button 
                  onClick={() => setAnalyticsView('Monthly')}
                  className={cn("text-sm font-medium transition-colors", analyticsView === 'Monthly' ? "text-slate-800" : "text-slate-400 hover:text-slate-600")}
                >
                  Monthly
                </button>
              </div>
          ...
          <button 
            ...
            className={cn(
              ...
              accessLevel === 'Admin' ? "bg-slate-200 text-slate-700 hover:bg-slate-300" : "bg-slate-100 text-slate-400 cursor-not-allowed"
            )}
          >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Telegram Bot Token</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Telegram Chat ID</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Watchdog Timeout (Mins)</label>
                    <input 
                      type="number" 
                      value={config.watchdog_timeout_min || ''}
                      onChange={e => {
                        setConfig({...config, watchdog_timeout_min: e.target.value});
                        handleSaveConfig('watchdog_timeout_min', e.target.value);
                      }}
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Safety Timeout (Mins)</label>
                    <input 
                      type="number" 
                      value={config.safety_timeout_min || ''}
                      onChange={e => {
                        setConfig({...config, safety_timeout_min: e.target.value});
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Shift Start (Hour)</label>
                      <input 
                        type="number" 
                        value={config.shift_start_hour || ''}
                        onChange={e => {
                          setConfig({...config, shift_start_hour: e.target.value});
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Shift End (Hour)</label>
                      <input 
                        type="number" 
                        value={config.shift_end_hour || ''}
                        onChange={e => {
                          setConfig({...config, shift_end_hour: e.target.value});
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-start justify-between mb-6 shrink-0">
                <div>
                  <h3 className="text-xl font-black text-slate-800">Detailed Report</h3>
                  <div className="mt-3 flex items-center gap-2">
                    <label className="text-sm text-slate-500 font-medium">Select Date:</label>
                    <input 
                      type="date" 
                ...
                <button onClick={() => setShowDetailedReport(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><X className="w-5 h-5 text-slate-400" /></button>
              </div>
                    ...
                    <table className="w-full text-left text-sm">
                      <thead className="bg-slate-100/50 text-slate-500 text-[10px] uppercase tracking-widest sticky top-0 backdrop-blur-md">
                        <tr>
                          ...
                          <th className="px-4 py-3 font-medium text-right">Working Time</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100">
                        {detailedReportData.map((log, i) => (
                          <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-4 py-3 font-mono text-slate-700">
                              {new Date(log.onTime).toLocaleTimeString('en-US', { hour12: false })}
                            </td>
                            <td className="px-4 py-3 font-mono text-slate-700">
                              {new Date(log.offTime).toLocaleTimeString('en-US', { hour12: false })}
                            </td>
                      ...
                      <tfoot className="bg-slate-100/50 font-bold text-slate-900 border-t-2 border-slate-200 sticky bottom-0">
                        <tr>
                          <td colSpan={2} className="px-4 py-3 text-right uppercase tracking-widest text-[11px] text-slate-500">Total</td>
                          <td className="px-4 py-3 font-mono text-right">
                            {formatDuration(detailedReportData.reduce((acc, curr) => acc + curr.durationMinutes, 0))}
                          </td>
                        </tr>
                      </tfoot>
                    </table>
        ...
        <AnimatePresence>
          {showLogModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-black text-slate-800">Add Maintenance Log</h3>
        ...
        <AnimatePresence>
          {showAddNodeModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-black text-slate-800">Add New Node</h3>
                  <button onClick={() => setShowAddNodeModal(false)}><X className="w-5 h-5 text-slate-400" /></button>
                </div>
                <form onSubmit={handleAddNode} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Node ID (Unique)</label>
                    <input 
                      required
                      type="text" 
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Alias / Name</label>
                    <input 
                      type="text" 
        ...
        <AnimatePresence>
          {nodeToDelete && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xl font-black text-slate-800">Delete Node</h3>
                  <button onClick={() => setNodeToDelete(null)}><X className="w-5 h-5 text-slate-400" /></button>
                </div>
                <div className="space-y-4">
                  <p className="text-sm text-slate-600">
                    Are you sure you want to delete node <span className="font-bold text-slate-800">{nodeToDelete}</span>?
                  </p>
                  <p className="text-xs text-red-500 bg-red-50 p-3 rounded-lg border border-red-100">
                    This action is permanent and will also delete all maintenance logs and notifications associated with this node.
                  </p>
