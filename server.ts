...
async function sendDailyReportEmail(settings: any, isTest: boolean = false) {
  ...
  let cleanHost = settings.smtp_host.trim();
  // Remove any trailing dot from host if present
  if (cleanHost.endsWith(".")) {
    cleanHost = cleanHost.slice(0, -1);
  }
  // Auto-correct common typo "gamil.com" to "gmail.com"
  if (cleanHost.toLowerCase() === "smtp.gamil.com") {
  ...
  // Generate report's date string in Asia/Kolkata timezone
  const nowISTStr = new Date().toLocaleString("en-US", {
    timeZone: "Asia/Kolkata",
  });
  const reportDateInIST = new Date(nowISTStr);
  ...
  if (hourVal < 12) {
    reportDateInIST.setDate(reportDateInIST.getDate() - 1);
  }

  const targetYear = reportDateInIST.getFullYear();
  const targetMonth = String(reportDateInIST.getMonth() + 1).padStart(2, "0");
  const targetDay = String(reportDateInIST.getDate()).padStart(2, "0");

  // Start and End in IST (+05:30) converted to UTC ISO strings
  let startOfDay = new Date(
    `${targetYear}-${targetMonth}-${targetDay}T00:00:00.000+05:30`
  ).toISOString();
  let endOfDay = new Date(
    `${targetYear}-${targetMonth}-${targetDay}T23:59:59.999+05:30`
  ).toISOString();
  let reportDateISTStr = `${targetDay}/${targetMonth}/${targetYear}`;

  // Compute 30-day range for Monthly Day-wise Summary
  const startDateInIST = new Date(
    reportDateInIST.getTime() - 29 * 24 * 60 * 60 * 1000
  );
  const startYear = startDateInIST.getFullYear();
  const startMonth = String(startDateInIST.getMonth() + 1).padStart(2, "0");
  const startDay = String(startDateInIST.getDate()).padStart(2, "0");

  let start30Day = new Date(
    `${startYear}-${startMonth}-${startDay}T00:00:00.000+05:30`
  ).toISOString();
  let end30Day = endOfDay;
  let start30DayStr = `${startDay}/${startMonth}/${startYear}`;
  let end30DayStr = reportDateISTStr;
  ...
  for (const node of nodes) {
    ...
    const monthlyLogs = db
      .prepare(
        `
      SELECT 
        date(on_time, '+5 hours', '+30 minutes') as ist_date,
        SUM(duration_minutes) as daily_minutes
      FROM telemetry
      WHERE device_id = ?
        AND on_time >= ?
        AND on_time <= ?
      GROUP BY date(on_time, '+5 hours', '+30 minutes')
      ORDER BY date(on_time, '+5 hours', '+30 minutes') ASC
    `
      )
      .all(node.id, start30Day, end30Day) as any[];

    monthlyReportData[node.id] = {
      nodeName: node.alias || node.id,
      logs: monthlyLogs,
    };
  ...
  if (!hasDailyData && isTest) {
    ...
    if (latestTelemetry && latestTelemetry.on_time) {
      ...
      const fallbackKolkataStr = fallbackDate.toLocaleString("en-US", {
        timeZone: "Asia/Kolkata",
      });
      const fallbackIST = new Date(fallbackKolkataStr);

      const fY = fallbackIST.getFullYear();
      const fM = String(fallbackIST.getMonth() + 1).padStart(2, "0");
      const fD = String(fallbackIST.getDate()).padStart(2, "0");

      startOfDay = new Date(
        `${fY}-${fM}-${fD}T00:00:00.000+05:30`
      ).toISOString();
      endOfDay = new Date(`${fY}-${fM}-${fD}T23:59:59.999+05:30`).toISOString();
      reportDateISTStr = `${fD}/${fM}/${fY}`;

      // Compute 30-day range for fallback
      const fallbackStart30IST = new Date(
        fallbackIST.getTime() - 29 * 24 * 60 * 60 * 1000
      );
      const fsY = fallbackStart30IST.getFullYear();
      const fsM = String(fallbackStart30IST.getMonth() + 1).padStart(2, "0");
      const fsD = String(fallbackStart30IST.getDate()).padStart(2, "0");

      start30Day = new Date(
        `${fsY}-${fsM}-${fsD}T00:00:00.000+05:30`
      ).toISOString();
      end30Day = endOfDay;
      start30DayStr = `${fsD}/${fsM}/${fsY}`;
      end30DayStr = reportDateISTStr;
      ...
      console.log(
        `Test email fallback active: Found latest telemetry data on ${reportDateISTStr} IST. Generating attachment for this date.`
      );
    }
  }
  ...
  for (const node of nodes) {
    const dailyInfo = dailyReportData[node.id];
    const monthlyInfo = monthlyReportData[node.id];

    const hasDaily = dailyInfo && dailyInfo.logs && dailyInfo.logs.length > 0;
    const hasMonthly =
      monthlyInfo && monthlyInfo.logs && monthlyInfo.logs.length > 0;

    if (!hasDaily && !hasMonthly) continue;
    hasData = true;

    // 1. If we have Daily Logs, add the Daily Details Worksheet
    if (hasDaily) {
      ...
      sheet.addRow([`Telemetry Report for Node: ${dailyInfo.nodeName}`]);
      sheet.addRow([`Date: ${reportDateISTStr}`]);
      sheet.addRow([]);
      ...
      dailyInfo.logs.forEach((log: any) => {
        const dObj = new Date(log.on_time);
        ...
        const onISTDateStr = `${day}/${month}/${year}`;

        const onISTTimeStr = new Date(log.on_time).toLocaleTimeString("en-US", {
          timeZone: "Asia/Kolkata",
        });
        const offISTTimeStr = new Date(log.off_time).toLocaleTimeString(
          "en-US",
          { timeZone: "Asia/Kolkata" }
        );
      ...
      const totalMinutes = dailyInfo.logs.reduce(
        (acc: number, curr: any) => acc + curr.duration_minutes,
        0
      );
    ...
    // 2. If we have Monthly Logs, add the Monthly Day-wise Summary Worksheet
    if (hasMonthly) {
      const sheetName = getSafeSheetName(
        monthlyInfo.nodeName,
        "Monthly",
        usedSheetNames
      );
      const sheet = workbook.addWorksheet(sheetName);

      sheet.addRow([`Telemetry Report for Node: ${monthlyInfo.nodeName}`]);
      sheet.addRow([`Date Range: ${start30DayStr} to ${end30DayStr}`]);
      sheet.addRow([]);
      ...
      monthlyInfo.logs.forEach((log: any) => {
        sheet.addRow([
          formatYMDToDMY(log.ist_date),
          formatDurationHHMMSS(log.daily_minutes),
          formatDurationInWords(log.daily_minutes),
        ]);
      });

      const totalMinutes = monthlyInfo.logs.reduce(
        (acc: number, curr: any) => acc + curr.daily_minutes,
        0
      );
      const totalRow = sheet.addRow([
        "Total",
  ...
  if (!hasData) {
    if (isTest) {
      ...
      const mailOptions = {
        ...
        to: settings.emails,
        subject: `Test Daily Telemetry Report - ${reportDateISTStr}`,
        text: `This is a test email from Factory Portal. There is no telemetry data for ${reportDateISTStr}, so no Excel file is attached.`,
      };
      const info = await transporter.sendMail(mailOptions);
      console.log("Test report email sent to", settings.emails);
      return info;
    }
    console.log(`No data for ${reportDateISTStr}. Skipping email report.`);
    return null;
  }

  const buffer = await workbook.xlsx.writeBuffer();
  const filenameDate = reportDateISTStr.replace(/\//g, "-");

  const mailOptions = {
    from: `"Factory Portal" <${settings.smtp_user}>`,
    to: settings.emails,
    subject: `Daily Telemetry Report - ${reportDateISTStr}`,
    text: "Please find the attached daily telemetry report.",
    attachments: [
      {
        filename: `Daily_Report_${filenameDate}.xlsx`,
...
function getSafeSheetName(
  ...
  const clean = nodeName.replace(/[\\/*?:\[\]]/g, "");
  const limit = 31 - suffix.length - 3; // leave space for " - "
  let baseName = `${clean.substring(0, limit)} - ${suffix}`;

  if (!usedSheetNames.has(baseName.toLowerCase())) {
    usedSheetNames.add(baseName.toLowerCase());
    return baseName;
  }
  ...
  while (true) {
    ...
    const newLimit = 31 - suffix.length - 3 - counterStr.length;
    const trialName = `${clean.substring(0, newLimit)} - ${suffix}${counterStr}`;
    if (!usedSheetNames.has(trialName.toLowerCase())) {
      usedSheetNames.add(trialName.toLowerCase());
      return trialName;
    }
    counter++;
  }
